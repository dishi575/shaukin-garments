...
export default function ProductPage() {
  ...
  if (isLoading) return (
    <div className="max-w-6xl mx-auto px-4 py-16 flex items-center justify-center">
      <div className="w-10 h-10 rounded-full border-2 animate-spin" style={{ borderColor: "var(--brand)", borderTopColor: "transparent" }} />
    </div>
  );
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-10">
      <div className="flex items-center gap-2 text-sm mb-8" style={{ color: "var(--text-secondary)" }}>
        <Link href="/catalogue" className="flex items-center gap-1 hover:underline"><ArrowLeft size={14} /> Catalogue</Link>
        <span>/</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
          ...
          <div className="relative rounded-2xl overflow-hidden mb-3" style={{ aspectRatio: "1", background: "var(--surface-2)" }}>
            {images.length > 0 ? (
                ...
                {images.length > 1 && (
                  <>
                    <button onClick={() => setImgIndex(i => (i - 1 + images.length) % images.length)}
                      className="absolute left-3 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full flex items-center justify-center" style={{ background: "rgba(255,255,255,0.9)" }}>
                      <ChevronLeft size={16} />
                    </button>
                    <button onClick={() => setImgIndex(i => (i + 1) % images.length)}
                      className="absolute right-3 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full flex items-center justify-center" style={{ background: "rgba(255,255,255,0.9)" }}>
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center gap-3" style={{ color: "var(--text-secondary)" }}>
                <Package size={48} /><span className="text-sm">No image available</span>
              </div>
            )}
          </div>
          {images.length > 1 && (
        ...
        <div>
          ...
          <div className="rounded-2xl p-5 mb-6" style={{ background: "var(--surface-2)", border: "1px solid var(--border)" }}>
            ...
            {product.is_bulk_available && <p className="text-xs" style={{ color: "var(--text-secondary)" }}>Bulk pricing from MOQ of {product.moq} pieces</p>}
          </div>
          ...
          {product.available_sizes?.length > 0 && (
            <div className="mb-5">
              ...
              <div className="flex flex-wrap gap-2">
                {product.available_sizes.map((s: string) => <span key={s} className="px-3 py-1 rounded-lg text-sm border" style={{ borderColor: "var(--border)" }}>{s}</span>)}
              </div>
            </div>
          )}
          ...
          {product.available_colors?.length > 0 && (
            <div className="mb-5">
              ...
              <div className="flex flex-wrap gap-2">
                {product.available_colors.map((c: string) => <span key={c} className="px-3 py-1 rounded-lg text-sm border" style={{ borderColor: "var(--border)" }}>{c}</span>)}
              </div>
            </div>
          )}
          ...
          <div className="flex items-center gap-2 mb-6">
            ...
            <span className="text-sm" style={{ color: "var(--text-secondary)" }}>{product.stock > 0 ? `${product.stock} in stock` : "Out of stock"}</span>
          </div>
            ...
            {product.is_retail_available && (
              <div className="flex gap-3">
                <div className="flex items-center border rounded-xl overflow-hidden" style={{ borderColor: "var(--border)" }}>
                  <button onClick={() => setQty(q => Math.max(1, q - 1))} className="px-3 py-3 text-lg hover:bg-gray-50">−</button>
                  <span className="px-4 py-3 text-sm font-medium min-w-[3rem] text-center">{qty}</span>
                  <button onClick={() => setQty(q => q + 1)} className="px-3 py-3 text-lg hover:bg-gray-50">+</button>
                </div>
                <button onClick={handleAddToCart}
                  ...
                  style={{ background: added ? "var(--success)" : "var(--brand)" }}>
